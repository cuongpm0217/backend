...
public class CustomerListener {
	...
	private void afterCreate(Customer target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Customer target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Customer target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Customer.class.getName(), action, String.valueOf(id)));
	}
}
