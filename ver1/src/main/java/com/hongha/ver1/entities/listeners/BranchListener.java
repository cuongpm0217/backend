...
public class BranchListener {
	...
	private void afterCreate(Branch target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Branch target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Branch target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Branch.class.getName(), action, String.valueOf(id)));
	}
}
