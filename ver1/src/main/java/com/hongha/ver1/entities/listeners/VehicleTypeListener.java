...
public class VehicleTypeListener {
	...
	private void afterCreate(VehicleType target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(VehicleType target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(VehicleType target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(VehicleType.class.getName(), action, String.valueOf(id)));
	}
}
