...
public class SalaryListener {
	...
	private void afterCreate(Salary target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Salary target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Salary target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Salary.class.getName(), action, String.valueOf(id)));
	}
}
