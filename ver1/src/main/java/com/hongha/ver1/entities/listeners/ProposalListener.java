...
public class ProposalListener {
	...
	private void afterCreate(Proposal target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Proposal target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Proposal target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Proposal.class.getName(), action, String.valueOf(id)));
	}
}
