...
import com.hongha.ver1.entities.ProductCategory;
...
public class ProducCategorytListener {
	@PostPersist
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterCreate(ProductCategory target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(ProductCategory target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(ProductCategory target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(ProductCategory.class.getName(), action, String.valueOf(id)));
	}
}
