...
public class PositionListener {
	...
	private void afterCreate(Position target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Position target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Position target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Position.class.getName(), action, String.valueOf(id)));
	}
}
