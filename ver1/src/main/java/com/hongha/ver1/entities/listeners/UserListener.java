...
public class UserListener {
	...
	private void afterCreate(User target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(User target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(User target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(User.class.getName(), action, String.valueOf(id)));
	}
}
