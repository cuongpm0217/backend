...
public class ProductListener {
	...
	private void afterCreate(Product target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Product target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Product target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Product.class.getName(), action, String.valueOf(id)));
	}
}
