...
public class LedgerListener {
	...
	private void afterCreate(Ledger target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Ledger target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Ledger target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Ledger.class.getName(), action, String.valueOf(id)));
	}
}
