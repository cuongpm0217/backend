...
public class EmployeeListener {
	...
	private void afterCreate(Employee target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Employee target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Employee target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Employee.class.getName(), action, String.valueOf(id)));
	}
}
