...
public class DepartmentListener {
	...
	private void afterCreate(Department target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Department target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Department target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Department.class.getName(), action, String.valueOf(id)));
	}
}
