...
public class ProductTypeListener {
	...
	private void afterCreate(ProductType target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(ProductType target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(ProductType target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(ProductType.class.getName(), action, String.valueOf(id)));
	}
}
