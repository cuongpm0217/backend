...
public class UnitListener {
	...
	private void afterCreate(Unit target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Unit target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Unit target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Unit.class.getName(), action, String.valueOf(id)));
	}
}
