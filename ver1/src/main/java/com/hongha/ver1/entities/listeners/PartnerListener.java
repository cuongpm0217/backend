...
public class PartnerListener {
	...
	private void afterCreate(Partner target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Partner target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Partner target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Partner.class.getName(), action, String.valueOf(id)));
	}
}
