...
public class CurrencyListener {
	...
	private void afterCreate(Currency target) {
		perform(EAction.CREATE, target.getId());
	}

	@PostUpdate
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterUpdate(Currency target) {
	...
	@PostRemove
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	private void afterRemove(Currency target) {
	...
	private void perform(EAction action, Long id) {
		...
		entityManager.persist(new History(Currency.class.getName(), action, String.valueOf(id)));
	}
}
